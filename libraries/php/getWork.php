...
function imageTitleList() {
    ...
    foreach ( $imgs['data'] as $index => $value)
    {
        if ($imgs['meta'][$index]['featured'][0] == 'yes') {
            ...
            $o .= '<li>';
            $o .= '<a class="overlayLink" href="/partial/students/view/overlay/options/id/' . $id . '/image/' . $index . '">' . $value['post_title'] . '</a>';
            $o .= '</li>';
...
function drawAllImages() {
    ...
    foreach ( $imgs['data'] as $index => $value)
    {
        if ($imgs['meta'][$index]['featured'][0] == 'yes' && !empty($imgs['sizes'][$index]['sizes']['thumbnail'])) {
            ...
            $o .= '  <div class="thumbnail" data-course="'. $course .'" data-medium="'. $imgs['meta'][$index]['medium'][0] .'" title="' . $imgs['data'][$index]['post_title'] . '&lt;br&gt;' . $imgs['author'][$index]->first_name . ' ' . $imgs['author'][$index]->last_name . '">
                        <a href="/students/view/options/action/overlay/id/' . $imgs['author'][$index]->ID . '/image/' . $imgs['data'][$index]['ID'] . '" style=" background-image: url(/upload/files/' . $imgs['author'][$index]->user_nicename . '/' . $img_thumb['file'] . ');"></a>
                    </div>';
        }
    }
    return $o;
...
function overlayProjectThumbs($i) {
    ...
    foreach ( $imgs['data'] as $index => $value)
    {
        if ($imgs['meta'][$index]['project'][0] == $project) {
            ...
            $o .= '<a href="/upload/files/' . $user->user_nicename . '/' . $imgs['sizes'][$index]['sizes']['large']['file'] . '" class="olthumblink"><span><img class="olthumbnail" style="width: ' . $img_med['width'] . '; height: ' . $img_med['height'] . '" src="/upload/files/' . $user->user_nicename . '/' . $img_med['file'] . '" /></span></a>';
        }
    }
    return $o;
}
